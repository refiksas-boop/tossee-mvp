...
function tossee_api_get_profile() {
    ...
    // FIX: Include photo field in the SELECT query
    $user = $wpdb->get_row(
        $wpdb->prepare("SELECT id, tossee_id, username, email, dob, photo, created_at FROM $table WHERE tossee_id = %s", $tossee_id)
    );
    ...
    return [
        ...
        'email' => $user->email,
        'dob' => $user->dob,
        'photo' => $user->photo,  // FIX: Include photo in response
